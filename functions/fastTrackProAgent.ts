...
async function verifyAlumniRoles(b44, alumni, tc) {
  ...
  const al = alumni.slice(0,8).map(a => ({ name:a.name, claimed_title:a.role_title, claimed_company:a.company, linkedin_url:a.linkedin_url||'' }));
  try {
    const v = await b44.integrations.Core.InvokeLLM({ prompt:`STRICT verifier. For each person, determine if they CURRENTLY work at "${tc}".\nRULES:\n- "high": clear recent evidence (2024-2026) they work at ${tc}\n- "medium": some connection but cannot confirm CURRENT employment\n- "low": no evidence, or evidence they work ELSEWHERE, or "Ex-${tc}"/former\n- Past employment ≠ current. AWS certs ≠ Amazon employee.\nPEOPLE:\n${JSON.stringify(al)}`, add_context_from_internet:true, model:'gemini_3_flash', response_json_schema:{ type:"object", properties:{ verifications:{ type:"array", items:{ type:"object", properties:{ name:{type:"string"}, confidence:{type:"string",enum:["high","medium","low"]}, actual_company:{type:"string"}, actual_title:{type:"string"}, reason:{type:"string"} }, required:["name","confidence","reason"] } } }, required:["verifications"] } });
    const vm = new Map(); (v.verifications||[]).forEach(x => vm.set(x.name?.toLowerCase()?.trim(), x));
    return alumni.map(a => { const x = vm.get(a.name?.toLowerCase()?.trim()); if (!x) return { ...a, confidence:'medium', verification_note:'Unverified — check LinkedIn' }; const r = { ...a, confidence:x.confidence, verification_note:x.reason||'' }; if (x.confidence==='low') { r.verification_note = (x.actual_company?`May work at ${x.actual_company}. `:'') + x.reason; r.match_score = Math.min(r.match_score||0,40); } if (x.confidence==='medium') r.match_score = Math.min(r.match_score||65,70); if (x.actual_title && x.confidence!=='low') r.role_title = x.actual_title; return r; });
  } catch(e) { console.log('Verification failed:', e.message); return alumni.map(a => ({ ...a, confidence:'medium', verification_note:'Verification unavailable' })); }
}
    ...
    if (detectedCompany) {
      ...
      try {
        ...
        if (fCached?.length > 0) { fAlumni = await crossReferenceCFF(base44, fCached.map(a => ({ name: a.name, role_title: a.role_title, company: a.company, match_score: Math.max(a.match_score||65,65), degree_info: a.degree_info, location: a.location, linkedin_url: a.linkedin_url||'', verified: a.verified||false }))); }
          ...
          if (rawA.length > 0) { rawA = (await verifyAlumniRoles(base44, rawA, detectedCompany)).filter(a => a.confidence !== 'low'); }
