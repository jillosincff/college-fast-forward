...
${String(typeof webResult === 'string' ? webResult : JSON.stringify(webResult)).substring(0,4000)}`,
    ...
    // 8.5 CAREER EVENTS
    if (detectCareerEventsQuery(resolvedMessage)) {
      console.log('Intent: career_events'); if(!upcomingEvents.length) return Response.json({success:true,response:"No upcoming UF events loaded. Check [careerhub.ufl.edu/events](https://careerhub.ufl.edu/events/)!",message_type:'text',payload:{suggested_actions:['Research my target companies →']}});
      const now=new Date(),evts=upcomingEvents.map(e=>{const s=new Date(e.start_date),dl=e.deadline?new Date(e.deadline):null,du=dl?Math.ceil((dl-now)/864e5):null;return{title:e.title,start_date:s.toISOString(),end_date:e.end_date||null,deadline:e.deadline||null,days_until_deadline:du,description:e.description||'',event_type:e.event_type,url:e.url||'',location:e.location||''};});
      let resp='Here are upcoming UF Career Center events:\n\n'; for(const e of evts){const s=new Date(e.start_date),ds=e.end_date?`${s.toLocaleDateString('en-US',{month:'short',day:'numeric'})}–${new Date(e.end_date).toLocaleDateString('en-US',{month:'short',day:'numeric'})}`:s.toLocaleDateString('en-US',{month:'short',day:'numeric'}),dls=e.days_until_deadline!=null?(e.days_until_deadline<0?' (Deadline passed)':e.days_until_deadline<=7?` — ⚠️ **Deadline in ${e.days_until_deadline} days!**`:` — Deadline: ${new Date(e.deadline).toLocaleDateString('en-US',{month:'short',day:'numeric'})}`):'' ;resp+=`• **${e.title}** — ${ds} — ${e.location}${dls}\n`;if(e.description)resp+=`  ${e.description}\n`;if(e.url)resp+=`  [Details →](${e.url})\n`;resp+='\n';} resp+=`*From UF Career Center — verify at [careerhub.ufl.edu](https://careerhub.ufl.edu/events/)*`;
      const sa=['Research my #1 target company →','Help me prep for a career fair →'];return Response.json({success:true,response:resp,message_type:'career_events',payload:{events:evts,suggested_actions:sa}});
    }
