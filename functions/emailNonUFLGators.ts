...
Deno.serve(async (req) => {
  try {
    ...
    // Find gators with non-UFL emails
    const gators = await base44.asServiceRole.entities.User.filter({ persona: 'gator' }, '-created_date', 200);
    
    const nonUFLGators = (gators || []).filter(u => {
