  ...
  return (
    <div style={{ background: '#fff', border: '1px solid #E0E0E0', borderRadius: 12, padding: 20, boxShadow: '0 1px 4px rgba(0,0,0,0.06)', display: 'flex', flexDirection: 'column', gap: 12 }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
        <div style={{ width: 44, height: 44, borderRadius: '50%', background: '#E85D20', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: 16, flexShrink: 0 }}>{initials}</div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <p style={{ fontWeight: 700, fontSize: 15, color: '#1A1A1A', margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{masked}</p>
          {parent.company
            ? <p style={{ fontSize: 13, color: '#666', margin: '2px 0 0' }}>{parent.company}{parent.job_title ? ` · ${parent.job_title}` : ''}</p>
            : parent.industry
            ? <p style={{ fontSize: 13, color: '#888', margin: '2px 0 0' }}>{parent.industry}</p>
            : <p style={{ fontSize: 12, color: '#aaa', margin: '2px 0 0', fontStyle: 'italic' }}>Parent</p>}
        </div>
      </div>
      {parent.industry && parent.company && (
        <span style={{ display: 'inline-block', background: '#FFF5F0', color: '#E85D20', fontSize: 11, fontWeight: 600, padding: '3px 10px', borderRadius: 100, border: '1px solid #FDDBC8', alignSelf: 'flex-start' }}>{parent.industry}</span>
      )}
      <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
        <span style={{ width: 8, height: 8, borderRadius: '50%', background: avail.dot, flexShrink: 0 }} />
        <span style={{ fontSize: 12, color: '#666' }}>{avail.label}</span>
      </div>
      {parent.linkedin_url && (
        <a href={parent.linkedin_url} target="_blank" rel="noopener noreferrer" style={{ fontSize: 13, color: '#E85D20', fontWeight: 500, display: 'flex', alignItems: 'center', gap: 4, textDecoration: 'none' }}>
          <Linkedin style={{ width: 13, height: 13 }} /> View LinkedIn →
        </a>
      )}
      {canMessage
        ? <button onClick={() => onMessage(parent)} className="w-full bg-[#E85D20] text-white py-2.5 rounded-full font-semibold hover:bg-[#d44e14] transition-colors" style={{ minHeight: 'auto', fontSize: 14 }}>Send a Message →</button>
        : <p style={{ fontSize: 13, color: '#999', textAlign: 'center', margin: 0 }}>Not accepting messages right now</p>}
    </div>
  );
}

function FreeTierDirectoryTab({ user, onOpenUpgrade }) {
  ...
  useEffect(() => {
    ...
    base44.entities.Message.filter({ sender_email: user.email, message_type: 'directory_initial' })
      .then(msgs => {
        ...
        setMonthlyCount(new Set(thisMonth.map(m => m.recipient_email)).size);
      }).catch(() => {});
  }, [user?.email]);
  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-8">
      <p style={{ fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.15em', color: '#E85D20', marginBottom: 12 }}>PARENT DIRECTORY</p>
      <h1 style={{ fontSize: 32, fontWeight: 700, color: '#1A1A1A', marginBottom: 6 }}>Find someone who can open a door.</h1>
      <p style={{ fontSize: 15, color: '#666', marginBottom: 24 }}>Browse parents and professionals in the College Fast Forward network.</p>
      {isFree && monthlyCount >= FREE_LIMIT && (
        <div className="bg-orange-50 border border-orange-200 rounded-xl p-4 mb-6">
          <p style={{ fontSize: 14, fontWeight: 600, color: '#E85D20', marginBottom: 4 }}>You've reached your free messaging limit for this month.</p>
          <p style={{ fontSize: 13, color: '#666', marginBottom: 12 }}>Upgrade to FastIQ for unlimited access to the parent network.</p>
          <button onClick={onOpenUpgrade} className="bg-[#E85D20] text-white px-6 py-2 rounded-full font-semibold text-sm" style={{ minHeight: 'auto' }}>Unlock FastIQ →</button>
        </div>
      )}
      <div className="relative mb-4">
        <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-[#999]" />
        <input type="text" placeholder="Search by industry, company, or name..." value={search} onChange={e => setSearch(e.target.value)} className="w-full pl-12 pr-4 py-3 rounded-full border border-[#E0E0E0] bg-white text-[#1A1A1A]" />
      </div>
      <div className="flex gap-2 mb-6 flex-wrap">
        {FILTERS.map(f => (
          <button key={f} onClick={() => setFilter(f)} className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${filter === f ? 'bg-[#E85D20] text-white' : 'bg-white text-[#666] border border-[#E0E0E0]'}`} style={{ minHeight: 'auto' }}>{f}</button>
        ))}
      </div>
      {loading ? (
        ...
        <div className="bg-white rounded-xl p-12 border border-[#E0E0E0] text-center">
          <p style={{ fontSize: 15, color: '#666', marginBottom: 8 }}>{search || filter !== 'All' ? 'No professionals found matching your search.' : 'The network is growing every day.'}</p>
          {(search || filter !== 'All') && <button onClick={() => { setSearch(''); setFilter('All'); }} style={{ fontSize: 13, color: '#E85D20', background: 'none', border: 'none', cursor: 'pointer', minHeight: 'auto' }}>Clear Search →</button>}
        </div>
      ) : (
        <div className="grid md:grid-cols-2 gap-4">
          {filtered.map(p => (
            <ParentCard key={p.id} parent={p} onMessage={parent => { if (isFree && monthlyCount >= FREE_LIMIT) { onOpenUpgrade(); return; } setSelectedParent(parent); }} />
          ))}
        </div>
      )}
      {selectedParent && (
        <ParentMessageComposer user={user} parent={selectedParent} onClose={() => setSelectedParent(null)} onSent={() => { setSelectedParent(null); setMonthlyCount(c => c + 1); }} />
      )}
    </div>
  );
}
