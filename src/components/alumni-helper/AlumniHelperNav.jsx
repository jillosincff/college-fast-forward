...
const NAV_LINKS = [
  { label: 'Dashboard', page: 'AlumniDashboard' },
  { label: 'Ask the Network', page: 'Connections' },
  { label: 'Directory', page: 'GatorDirectory' },
  { label: 'Messages', page: 'MyMessages' },
  { label: 'My Impact', page: 'MyImpact' },
];

export default function AlumniHelperNav({ user, currentPage = 'AlumniDashboard' }) {
  ...
  return (
    <nav style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 32 }}>
        <button onClick={() => navigate('AlumniDashboard')} style={{
          ...
          {NAV_LINKS.map(l => {
            ...
            return (
              <button key={l.page} onClick={() => navigate(l.page)} style={{
                ...
                borderBottom: active ? '2px solid #E85D20' : '2px solid transparent',
                ...
                onMouseLeave={e => { if (!active) e.currentTarget.style.color = active ? '#f4f0e8' : 'rgba(244,240,232,0.45)'; }}
              >
                {l.label}
              </button>
            );
      ...
      {mobileOpen && (
        <div className="md:hidden" style={{
          ...
          {NAV_LINKS.map(l => {
            const mActive = l.page === currentPage;
            return (
