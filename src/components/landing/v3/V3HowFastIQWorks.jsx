        ...
        <div className="v3-steps-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, ...fadeStyle(vis, 0.12) }}>
          {STEPS.map((s) => (
            <div key={s.num} style={{
              ...
              onMouseEnter={(e) => { e.currentTarget.style.borderColor = 'var(--accent-border, rgba(79,140,255,0.30))'; e.currentTarget.style.boxShadow = '0 0 24px var(--accent-glow, rgba(79,140,255,0.08))'; }}
              onMouseLeave={(e) => { e.currentTarget.style.borderColor = '#1F1F23'; e.currentTarget.style.boxShadow = 'none'; }}
            >
              <div className="flex items-center gap-3 mb-4">
                <div style={{ width: 44, height: 44, borderRadius: 12, background: 'var(--accent-soft, rgba(79,140,255,0.1))', border: '1px solid var(--accent-border, rgba(79,140,255,0.2))', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'background 0.4s, border-color 0.4s' }}>
                  {s.icon}
                </div>
