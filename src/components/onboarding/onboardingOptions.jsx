// Industries (shared by all roles)
export const INDUSTRIES = [
  { id: 'tech', label: 'Tech', emoji: '💻' },
  { id: 'finance', label: 'Finance', emoji: '💰' },
  { id: 'consulting', label: 'Consulting', emoji: '📊' },
  { id: 'healthcare', label: 'Healthcare', emoji: '🏥' },
  { id: 'marketing', label: 'Marketing', emoji: '📱' },
  { id: 'engineering', label: 'Engineering', emoji: '⚙️' },
  { id: 'law', label: 'Law', emoji: '⚖️' },
  { id: 'education', label: 'Education', emoji: '📚' },
  { id: 'real_estate', label: 'Real Estate', emoji: '🏠' },
  { id: 'nonprofit', label: 'Nonprofit', emoji: '❤️' },
  { id: 'government', label: 'Government', emoji: '🏛️' },
  { id: 'media', label: 'Media/Entertainment', emoji: '🎬' },
  { id: 'startups', label: 'Startups', emoji: '🚀' },
  { id: 'other', label: 'Other', emoji: '✨' },
];

// What students are looking for
export const STUDENT_SEEKING = [
  { id: 'fulltime', label: 'Full-time job', emoji: '💼' },
  { id: 'internship', label: 'Internship', emoji: '📋' },
  { id: 'coop', label: 'Co-op', emoji: '🔄' },
  { id: 'parttime', label: 'Part-time', emoji: '⏰' },
  { id: 'gradschool', label: 'Grad school advice', emoji: '🎓' },
  { id: 'exploring', label: 'Just exploring', emoji: '🧭' },
];

// What help students need
export const STUDENT_HELP_NEEDED = [
  { id: 'job_search', label: 'Finding jobs/internships', emoji: '🔍' },
  { id: 'resume', label: 'Resume & LinkedIn help', emoji: '📝' },
  { id: 'interviews', label: 'Interview prep', emoji: '🎤' },
  { id: 'salary', label: 'Salary negotiation', emoji: '💰' },
  { id: 'networking', label: 'Networking & connections', emoji: '🤝' },
  { id: 'direction', label: 'Figuring out what I want', emoji: '🧭' },
  { id: 'industry_insights', label: 'Industry insights', emoji: '💼' },
  { id: 'grad_school', label: 'Grad school advice', emoji: '🎓' },
];

// How parents/alumni can help (consolidated 5 categories)
export const EXPERTISE_AREAS = [
  { id: 'career_guidance', label: 'Career guidance', emoji: '💼', description: 'Career paths, transitions, mentorship, salary advice' },
  { id: 'jobs_referrals', label: 'Jobs & referrals', emoji: '🔍', description: 'Job search help, referrals, sharing opportunities' },
  { id: 'resume_interviews', label: 'Resume & interviews', emoji: '📄', description: 'Resume review, LinkedIn optimization, mock interviews' },
  { id: 'industry_insights', label: 'Industry insights', emoji: '🏢', description: 'Day-to-day work, breaking into a field, career paths' },
  { id: 'introductions', label: 'Introductions', emoji: '🤝', description: 'Connecting with specific people or companies' },
];

// Graduation years
export const GRAD_YEARS = ['2026', '2027', '2028', '2029', '2030'];