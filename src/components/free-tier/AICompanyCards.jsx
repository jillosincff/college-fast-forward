import React from 'react';
import { Loader2, RefreshCw } from 'lucide-react';

const SIGNAL_CONFIG = {
  hot:  { emoji: '🟢', label: 'Actively Hiring', bg: 'bg-green-900/40',  text: 'text-green-400' },
  warm: { emoji: '🟡', label: 'Selective',        bg: 'bg-yellow-900/40', text: 'text-yellow-400' },
  cool: { emoji: '🔴', label: 'Freeze',            bg: 'bg-red-900/40',   text: 'text-red-400' },
};

function SkeletonCard() {
  return (
    <div style={{ background: '#1A1A1A', border: '1px solid #2A2A2A', borderRadius: 12, padding: 16 }}>
      <div style={{ background: '#2A2A2A', borderRadius: 6, height: 18, width: '55%', marginBottom: 10, animation: 'pulse 1.5s infinite' }} />
      <div style={{ background: '#2A2A2A', borderRadius: 100, height: 22, width: '40%', marginBottom: 10, animation: 'pulse 1.5s infinite' }} />
      <div style={{ background: '#2A2A2A', borderRadius: 4, height: 12, width: '90%', marginBottom: 6, animation: 'pulse 1.5s infinite' }} />
      <div style={{ background: '#2A2A2A', borderRadius: 4, height: 12, width: '70%', animation: 'pulse 1.5s infinite' }} />
    </div>
  );
}

export default function AICompanyCards({ companies, loading, error, onRefetch, onTabChange, dark = true }) {
  const cardBg     = dark ? '#1A1A1A' : '#ffffff';
  const cardBorder = dark ? '#2A2A2A' : '#E0E0E0';
  const nameColor  = dark ? '#ffffff' : '#1A1A1A';
  const descColor  = dark ? '#888888' : '#666666';
  const whyColor   = dark ? '#999999' : '#888888';

  if (loading) {
    return (
      <div>
        <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 12, color: '#999', marginBottom: 12 }}>
          FastIQ is finding companies that match your goals...
        </p>
        <div className="grid md:grid-cols-3 gap-4">
          <SkeletonCard /><SkeletonCard /><SkeletonCard />
        </div>
      </div>
    );
  }

  if (error || !companies) {
    return (
      <div style={{ background: dark ? '#1A1A1A' : '#F9F9F9', border: `1px solid ${cardBorder}`, borderRadius: 12, padding: 20, textAlign: 'center' }}>
        <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 14, color: '#888', marginBottom: 12 }}>
          We're having trouble finding companies right now. Check back in a few minutes.
        </p>
        <button
          onClick={onRefetch}
          className="flex items-center gap-2 mx-auto text-[#E85D20] font-medium"
          style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 14, minHeight: 'auto' }}
        >
          <RefreshCw style={{ width: 14, height: 14 }} /> Retry →
        </button>
      </div>
    );
  }

  return (
    <div className="grid md:grid-cols-3 gap-4">
      {companies.slice(0, 3).map(company => {
        const sig = SIGNAL_CONFIG[company.hiring_signal] || SIGNAL_CONFIG.warm;
        return (
          <div key={company.name} style={{ background: cardBg, border: `1px solid ${cardBorder}`, borderRadius: 12, padding: 16 }}>
            <h3 style={{ fontWeight: 700, color: nameColor, marginBottom: 4, fontSize: 15 }}>{company.name}</h3>
            {company.industry && (
              <p style={{ fontSize: 11, color: whyColor, margin: '0 0 8px' }}>{company.industry}</p>
            )}
            <div className="flex items-center gap-2 mb-3">
              <span className={`text-xs px-2 py-1 rounded-full ${sig.bg} ${sig.text}`}>
                {sig.emoji} {sig.label}
              </span>
            </div>
            {company.hiring_description && (
              <p style={{ fontSize: 12, color: descColor, marginBottom: 8, lineHeight: 1.5 }}>{company.hiring_description}</p>
            )}
            {company.why_recommended && (
              <p style={{ fontSize: 11, fontStyle: 'italic', color: whyColor, marginBottom: 12, lineHeight: 1.5 }}>{company.why_recommended}</p>
            )}
            <button
              onClick={() => onTabChange && onTabChange('company_intel')}
              style={{ fontSize: 12, color: '#E85D20', fontWeight: 500, background: 'none', border: 'none', cursor: 'pointer', minHeight: 'auto', padding: 0 }}
            >
              View Full Intel →
            </button>
          </div>
        );
      })}
    </div>
  );
}