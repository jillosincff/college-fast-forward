    ...
    return (
      <div style={{ maxWidth: 720, margin: '0 auto', padding: '40px 24px' }}>
        ...
        <div style={{ display: 'flex', gap: 12 }}>
          <button onClick={() => onTargetConfirmed(recommendedTarget)} style={{ flex: 1, background: 'linear-gradient(135deg, #7c3aed, #4f46e5)', border: 'none', borderRadius: 12, padding: '16px 28px', fontSize: 14, fontWeight: 700, color: '#fff', cursor: 'pointer', fontFamily: dm, boxShadow: '0 4px 12px rgba(124,58,237,0.3)' }}>⚡ Craft Personalized Message</button>
          <a href={recommendedTarget.recommendedTarget?.linkedinUrl || `https://www.linkedin.com/search/results/people/?keywords=${encodeURIComponent(recommendedTarget.recommendedTarget?.name || '')}&company=${encodeURIComponent(company || '')}`} target="_blank" rel="noopener noreferrer" style={{ background: '#fff', border: '2px solid #0077B5', borderRadius: 12, padding: '15px 24px', fontSize: 13, fontWeight: 700, color: '#0077B5', cursor: 'pointer', fontFamily: dm, textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: 8 }}>🔍 Find on LinkedIn →</a>
        </div>
        <style>{`@keyframes spin { from { transform: rotate(0deg); } to { transform: rotate(360deg); } }`}</style>
      </div>
    );
