...
const SOURCE_CATEGORY_CONFIG = {
  A: { emoji: '🔥', label: 'HIDDEN NETWORK REFERRAL', bg: 'linear-gradient(135deg, #fff1f2, #fff5f5)', border: '#fecaca', color: '#991b1b' },
  B: { emoji: '🛰️', label: 'SOURCED VIA LINKEDIN MENTION', bg: 'linear-gradient(135deg, #f0f9ff, #e0f2fe)', border: '#bae6fd', color: '#075985' },
  C: { emoji: '⚡', label: 'DIRECT COMPANY BACKDOOR TRACK', bg: 'linear-gradient(135deg, #f5f3ff, #ede9fe)', border: '#ddd6fe', color: '#5b21b6' },
  D: { emoji: '💬', label: 'SOURCED FROM INDUSTRY THREADS', bg: 'linear-gradient(135deg, #fff7ed, #fef3c7)', border: '#fcd34d', color: '#92400e' },
};
...
function SourceCategoryBadge({ category, source }) {
  ...
  return (
    <div style={{ borderRadius: 8, background: cfg.bg, border: `1px solid ${cfg.border}`, padding: '8px 10px' }}>
      ...
      {source && <p style={{ fontFamily: dm, fontSize: 9, color: '#9ca3af', margin: '4px 0 0' }}>{source}</p>}
    </div>
  );
}
...
function buildMatchReasons(match, shortName) {
  ...
  if (alumCount > 0) reasons.push(`${alumCount} verified ${shortName} alumni at this company`);
  if (parentCount > 0) reasons.push(`${parentCount} ${shortName} parent${parentCount > 1 ? 's' : ''} opted in for referrals`);
  reasons.push('Warm path confirmed via CFF network');
  return reasons;
}
...
export default function MatchDeepDiveModal({ match, shortName, onClose, onGenerateOutreach, user }) {
  ...
  useEffect(() => {
    ...
    const mappedParents = parentsRaw.map(m => ({
      name: m.full_name || m.name || '',
      title: m.title || m.role_title || '',
  ...
  const handleOpenEmailClient = () => {
    ...
    const to = (contactEmail && contactEmail !== 'not_found') ? contactEmail : '';
  ...
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', zIndex: 60000, display: 'flex', alignItems: 'flex-end', justifyContent: 'center', backdropFilter: 'blur(4px)' }}>
      <div onClick={e => e.stopPropagation()} style={{ background: '#fff', borderRadius: '24px 24px 0 0', width: '100%', maxWidth: 560, maxHeight: '92vh', display: 'flex', flexDirection: 'column', boxShadow: '0 -8px 48px rgba(0,0,0,0.2)' }}>
        <style>{`@keyframes slideUp { from { transform: translateY(100%); opacity: 0; } to { transform: translateY(0); opacity: 1; } }`}</style>
        <div style={{ padding: '12px 0 4px', display: 'flex', justifyContent: 'center' }}><div style={{ width: 40, height: 4, borderRadius: 2, background: '#e2e8f0' }} /></div>
        <div ref={scrollContainerRef} style={{ overflowY: 'auto', flex: 1, padding: '0 20px 20px' }}>
          ...
          <div style={{ paddingTop: 4, paddingBottom: 20, borderBottom: '1px solid #f1f5f9' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 14 }}>
              <div style={{ width: 44, height: 44, borderRadius: 12, background: '#f8fafc', border: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22, flexShrink: 0 }}>{match.logo}</div>
              <div style={{ flex: 1 }}>
              ...
              <button onClick={onClose} style={{ background: '#f1f5f9', border: 'none', borderRadius: 8, width: 32, height: 32, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', fontSize: 16, color: '#64748b', minHeight: 'auto', flexShrink: 0 }}>✕</button>
            </div>
            <div style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 12, padding: '12px 14px', marginBottom: 14 }}>
              ...
              <p style={{ fontFamily: dm, fontSize: 13, color: '#374151', margin: '0 0 10px', lineHeight: 1.6 }}>{match.jobDescription || 'Role details coming soon.'}</p>
            ...
            <div style={{ background: 'linear-gradient(135deg, #f5f3ff, #eff6ff)', border: '1px solid #c7d2fe', borderRadius: 14, padding: '14px 16px', marginBottom: 14 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                ...
                <p style={{ fontFamily: dm, fontSize: 13, fontWeight: 800, color: '#4c1d95', margin: 0 }}>Network Connection Weight: {match.networkWeight || match.matchPct || 0}%</p>
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                {reasons.map((r, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <div style={{ width: 16, height: 16, borderRadius: '50%', background: '#7c3aed', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}><span style={{ fontSize: 9, color: '#fff' }}>✓</span></div>
          ...
          <div style={{ paddingTop: 20, paddingBottom: 20, borderBottom: '1px solid #f1f5f9' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
              <p style={{ fontFamily: dm, fontSize: 13, fontWeight: 800, color: '#0f172a', margin: 0 }}>🔓 Your Warm Network</p>
            ...
            <div style={{ display: 'flex', gap: 4, background: '#f8fafc', borderRadius: 10, padding: 4, marginBottom: 14 }}>
              {['alumni', 'parents'].map(t => (
                <button key={t} onClick={() => { setTab(t); setSelectedContact(null); }} style={{ flex: 1, fontFamily: dm, fontSize: 12, fontWeight: 700, color: tab === t ? '#fff' : '#64748b', background: tab === t ? (t === 'alumni' ? '#2563eb' : '#7c3aed') : 'transparent', border: 'none', borderRadius: 7, padding: '7px 0', cursor: 'pointer', minHeight: 'auto' }}>
            ...
            {loading ? (
              <div style={{ padding: '24px 0', textAlign: 'center' }}>
                ...
                <p style={{ fontFamily: dm, fontSize: 12, color: '#94a3b8', margin: 0 }}>Loading...</p>
              ...
              <div style={{ padding: '16px', background: 'linear-gradient(135deg, #faf5ff, #f0f9ff)', border: '1px solid #e9d5ff', borderRadius: 12, textAlign: 'center' }}>
                ...
                <p style={{ fontFamily: dm, fontSize: 12, color: '#7c3aed', margin: '0 0 12px', lineHeight: 1.5 }}>CLiFF will craft a custom cold inroad for this company.</p>
                <button onClick={() => { onClose(); setTimeout(() => { window.location.hash = `#OutreachDrafts?context=cold_outreach&company=${encodeURIComponent(match.company)}&role=${encodeURIComponent(match.role)}`; }, 200); }} style={{ fontFamily: dm, fontSize: 12, fontWeight: 800, color: '#fff', background: 'linear-gradient(135deg, #7c3aed, #4f46e5)', border: 'none', borderRadius: 10, padding: '10px 20px', cursor: 'pointer', minHeight: 'auto', boxShadow: '0 4px 12px rgba(124,58,237,0.35)' }}>⚡ Generate Cold Inroad</button>
              </div>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {contacts.map((c, i) => {
                  ...
                  return (
                    <div key={i} onClick={() => setSelectedContact(isSelected ? null : c)} style={{ display: 'flex', alignItems: 'center', gap: 12, background: isSelected ? (isAlum ? '#eff6ff' : '#f5f3ff') : '#f8fafc', border: `1.5px solid ${isSelected ? (isAlum ? '#93c5fd' : '#c4b5fd') : '#e2e8f0'}`, borderRadius: 12, padding: '12px 14px', cursor: 'pointer', transition: 'all 0.15s' }}>
                      <div style={{ width: 38, height: 38, borderRadius: '50%', flexShrink: 0, background: isAlum ? '#dbeafe' : '#ede9fe', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: dm, fontSize: 14, fontWeight: 800, color: isAlum ? '#2563eb' : '#7c3aed' }}>{(c.name || '?').split(' ').map(n => n[0]).join('').slice(0, 2)}</div>
            ...
            {selectedContact && !draftContact && <p style={{ fontFamily: dm, fontSize: 11, color: '#7c3aed', fontWeight: 600, margin: '10px 0 0', textAlign: 'center' }}>✓ {selectedContact.name} selected</p>}
            {draftContact && (
              <div style={{ marginTop: 14, background: '#f5f3ff', border: '1.5px solid #c4b5fd', borderRadius: 16, padding: '14px 16px', animation: 'slideUp 0.3s ease' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
                  ...
                  <button onClick={handleCloseDraft} style={{ background: '#ede9fe', border: 'none', borderRadius: 6, width: 24, height: 24, cursor: 'pointer', fontSize: 12, color: '#7c3aed', minHeight: 'auto', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✕</button>
                </div>
                {isGenerating ? (
                  ...
                  <>
                    ...
                    {outreachType === 'email' && contactEmail && contactEmail !== 'not_found' && (
                      <div style={{ background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: 8, padding: '8px 12px', marginTop: 10, display: 'flex', alignItems: 'center', gap: 6 }}>
                        <span style={{ fontSize: 13 }}>✉️</span>
                        <p style={{ fontFamily: dm, fontSize: 11, color: '#166534', margin: 0, fontWeight: 600 }}>Email found: {contactEmail}</p>
                      </div>
                    )}
