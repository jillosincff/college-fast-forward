...
Deno.serve(async (req) => {
  try {
    ...
    // Fetch the CSV file
    const csvUrl = 'https://media.base44.com/files/public/684474c5723dc90efce23588/768e4ac46_users_all_2026-04-25.csv';
    const csvRes = await fetch(csvUrl);
    const csvText = await csvRes.text();
    const rows = parseCSV(csvText);

    // Filter for UCF users
    const ucfRows = rows.filter(r => r['School']?.toLowerCase() === 'ucf');
      ...
      try {
        ...
        if (registered?.id) {
          ...
          await base44.asServiceRole.entities.User.update(registered.id, {
            ...
            roles: [persona],
            school_code: 'ucf',
            school_name: SCHOOL_NAMES['ucf'],
            school: SCHOOL_NAMES['ucf'],
    ...
    return Response.json({
      success: true,
      ucf_rows_in_csv: ucfRows.length,
      created,
      skipped,
