...
Deno.serve(async (req) => {
  try {
    ...
    // Load upcoming career events for this student's school (school-agnostic)
    let upcomingEvents = [], eventsContext = '';
    try {
      const studentSchoolCode = (user.school || user.university || 'UF').toUpperCase().substring(0, 10);
      const now = new Date(), ago7 = new Date(now.getTime() - 7*864e5);
      const allE = await base44.entities.SchoolCareerEvent.filter({ school_code: studentSchoolCode, is_active: true }, 'start_date', 20);
      upcomingEvents = (allE||[]).filter(e => {
        const s = new Date(e.start_date), d = e.deadline ? new Date(e.deadline) : null;
        return s > now || (d && d > ago7);
      }).slice(0, 5);
      console.log(`[Events] ${upcomingEvents.length} upcoming for school=${studentSchoolCode}`);
      if (upcomingEvents.length > 0) {
        const el = upcomingEvents.map(e => {
          const s = new Date(e.start_date), dl = e.deadline ? new Date(e.deadline) : null;
          const dd = dl ? Math.ceil((dl - now) / 864e5) : null;
          return `\u2022 ${e.title} \u2014 ${s.toLocaleDateString('en-US',{month:'short',day:'numeric'})}${dd !== null && dd >= 0 && dd <= 7 ? ' \u26a0\ufe0f DEADLINE ' + dd + 'd' : ''}`;
        }).join('\n');
        eventsContext = `\n\n${studentSchoolCode} CAREER EVENTS (surface 1-2 as brief addendum when relevant to job search/timelines, skip if not):\n${el}`;
      }
    } catch(e) {}
...
${String(typeof webResult === 'string' ? webResult : JSON.stringify(webResult)).substring(0,4000)}`,
    ...
    // 8.5 CAREER EVENTS
    if(detectCareerEventsQuery(resolvedMessage)){console.log('Intent: career_events');if(!upcomingEvents.length)return Response.json({success:true,response:"No upcoming UF events. Check [careerhub.ufl.edu/events](https://careerhub.ufl.edu/events/)!",message_type:'text',payload:{suggested_actions:['Research my target companies →']}});const now=new Date(),evts=upcomingEvents.map(e=>{const s=new Date(e.start_date),dl=e.deadline?new Date(e.deadline):null,du=dl?Math.ceil((dl-now)/864e5):null;return{title:e.title,start_date:s.toISOString(),end_date:e.end_date||null,deadline:e.deadline||null,days_until_deadline:du,description:e.description||'',event_type:e.event_type,url:e.url||'',location:e.location||''};});let resp='Here are upcoming UF Career Center events:\n\n';for(const e of evts){const s=new Date(e.start_date),ds=e.end_date?`${s.toLocaleDateString('en-US',{month:'short',day:'numeric'})}–${new Date(e.end_date).toLocaleDateString('en-US',{month:'short',day:'numeric'})}`:s.toLocaleDateString('en-US',{month:'short',day:'numeric'}),dls=e.days_until_deadline!=null?(e.days_until_deadline<0?' (Deadline passed)':e.days_until_deadline<=7?` — ⚠️ **Deadline in ${e.days_until_deadline} days!**`:` — Deadline: ${new Date(e.deadline).toLocaleDateString('en-US',{month:'short',day:'numeric'})}`):'' ;resp+=`• **${e.title}** — ${ds} — ${e.location}${dls}\n`;if(e.description)resp+=`  ${e.description}\n`;if(e.url)resp+=`  [Details →](${e.url})\n`;resp+='\n';}resp+=`*From UF Career Center — verify at [careerhub.ufl.edu](https://careerhub.ufl.edu/events/)*`;return Response.json({success:true,response:resp,message_type:'career_events',payload:{events:evts,suggested_actions:['Research my #1 target company →','Help me prep for a career fair →']}});}
    // 9. OPPORTUNITY DISCOVERY
    if (detectOpportunityDiscovery(resolvedMessage)) {
      console.log('Intent: opportunity_discovery'); const industry=profile.target_industry||'any industry',major=studentMajor,sizePref={large:'large corporation',mid_size:'mid-size company',startup:'startup'}[profile.company_size_preference]||'',locPref=profile.location_preference||'';
      if(targetCompanies.length===0&&(!profile.company_size_preference||!profile.location_preference)){let q=`Let's find the right companies for you! I already know you're interested in **${industry}**.`;if(!profile.company_size_preference)q+=`\n\nWhat size company appeals to you?\n- 🏢 **Big names** (Apple, Google, Disney)\n- 📈 **Mid-size** with strong growth\n- 🚀 **Startups** — wear many hats\n- 🤷 **Open to anything**`;if(!profile.location_preference)q+=`\n\nLocation preference?\n- 🌴 **Florida**\n- 🌎 **Open to relocating**\n- 📍 **Specific city** (tell me)`;q+=`\n\nJust tell me and I'll find companies hiring for your background!`;return Response.json({success:true,response:q,message_type:'text',payload:{}});}
      ...
      trackActivity(base44,user.email,profile.id,'company_search',industry); return Response.json({success:true,response:result.response||"Here are companies for you:",message_type:'company_suggestions',payload:{suggestions:result.suggestions||[]}});
    }
