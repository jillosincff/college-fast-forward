...
Deno.serve(async (req) => {
  try {
    ...
    const { userEmail, firstName } = await req.json();

    const html = emailWrapper(`
      ${darkHero(
        '📋 QUICK REMINDER',
        `${escapeHtml(firstName)}, you're one step away.`,
        `Your career goals unlock everything on CFF — and it takes 3 minutes.`
      )}
      ${bodySection(`
        ${bodyText(`You signed up for College Fast Forward but haven't set your career goals yet. No worries — we saved your spot.`)}
        ${bodyText(`Setting your goals is what makes CFF personal. Once you do, we'll:`)}
        ${featureList([
          '✅ Show you the right connections in our network',
          '✅ Surface alumni at your target companies',
          '✅ Score your resume against your specific roles',
          '✅ Generate outreach messages tailored to you',
        ])}
        ${ctaButton('Set My Career Goals Now', 'https://collegefastforward.com/#CareerGoals')}
        ${bodyText(`Takes 3 minutes. No fluff.`)}
      `)}
    `);
    ...
    const response = await fetch('https://api.sendgrid.com/v3/mail/send', {
      ...
      body: JSON.stringify({
        ...
        subject: `${firstName}, your career goals are waiting ⏳`,
