  ...
  <tr><td align="center">
    <table width="560" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:16px;overflow:hidden;max-width:560px;width:100%">
      <tr><td style="background:#0A0A0A;padding:28px 40px">
        ...
        <h1 style="margin:10px 0 0;font-size:24px;font-weight:700;color:#ffffff;line-height:1.3">${escapeHtml(firstName)}, it's time to follow up.</h1>
      </td></tr>
      <tr><td style="padding:32px 40px">
        <p style="margin:0 0 20px;font-size:15px;color:#444;line-height:1.7">It's been <strong>${escapeHtml(daysSinceSent)} days</strong> since you messaged <strong>${escapeHtml(recipientName)}</strong>${recipientTitle ? ` (${escapeHtml(recipientTitle)})` : ''} — and you haven't heard back.</p>
        ...
        <div style="background:#FAFAFA;border:1px solid #E0E0E0;border-radius:10px;padding:20px;margin-bottom:28px">
          <p style="margin:0;font-size:14px;color:#333;line-height:1.7;font-style:italic">"Hi ${escapeHtml(recipientName?.split(' ')[0] || 'there')}, just wanted to follow up on my message from last week. I know you're busy — even a 15-minute chat would mean a lot. Happy to work around your schedule."</p>
        </div>
        <div style="text-align:center;margin:8px 0 28px">
          <a href="${APP_URL}/#OutreachDrafts${draftId ? '?id=' + encodeURIComponent(draftId) : ''}" style="display:inline-block;background:#E85D20;color:#ffffff;text-decoration:none;font-size:14px;font-weight:600;padding:13px 32px;border-radius:100px">Send Follow-Up →</a>
        </div>
...
Deno.serve(async (req) => {
  ...
  const { to, firstName, recipientName, recipientTitle, daysSinceSent = 5, draftId, internalSecret } = await req.json();
  ...
  // Nudges go to the student themselves — never to an arbitrary address.
